...
function createWindow() {
  ...
  mainWindow.webContents.on('console-message', (event, level, message, line, sourceId) => {
    ...
    console.log(`[Renderer Console] [${label}] ${message} (at ${path.basename(sourceId)}:${line})`);
  });

  // 웹뷰가 부착(attach)되는 즉시 해당 웹뷰의 webContents에 팝업 및 쿠키 동기화 헬퍼 바인딩
  mainWindow.webContents.on('did-attach-webview', (event, webContents) => {
    console.log('[Main Process] WebView attached. Binding setWindowOpenHandler...');
  ...
  mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    console.error(`[Main Process] [ERROR] Failed to load URL: ${validatedURL}. Error: ${errorDescription} (${errorCode})`);
  });
...
function bindPopupAndCookieHandler(contents) {
  contents.setWindowOpenHandler((details) => {
    ...
    if (activePopups.has(contents.id)) {
      console.log(`[Main Process] window.open inside popup allowed for: ${details.url}`);
      return { action: 'allow' };
    }
    ...
    if (details.url && details.url !== 'about:blank') {
      try {
        ...
        if (isInternal) {
          console.log(`[Main Process] Internal URL detected, navigating parent view: ${details.url}`);
          setTimeout(() => {
    ...
    const handleNavigation = async (targetUrl) => {
      ...
      try {
        ...
        if (parentUrlStr) {
          ...
          const isOAuthProvider = parsedUrl.hostname.includes('google.com') ||
          ...
          isBackToApp = (targetRoot === parentRoot || 
                         parsedUrl.hostname === 'localhost' || 
                         parsedUrl.hostname === '127.0.0.1') && !isOAuthProvider;
        }
        
        // 팝업창 주소가 최종적으로 부모 웹뷰의 도메인 영역으로 안전하게 리다이렉션되어 복귀했는지 확인
        if (isBackToApp) {
          console.log(`[Main Process] OAuth Flow successfully completed. Final redirect target: ${targetUrl}`);
          ...
          if (!popupWin.isDestroyed()) {
            ...
            console.log(`[Main Process] Extracting ${cookies.length} cookies from OAuth popup session...`);
            ...
            for (const cookie of cookies) {
              ...
              try {
                ...
                console.log(`  [Cookie Sync] Successfully copied cookie: ${cookie.name} for domain ${cookie.domain}`);
              } catch (err) {
                console.error(`  [Cookie Sync] [ERROR] Failed to copy cookie: ${cookie.name}`, err);
              }
            }
            ...
            const verifiedCookies = await parentSession.cookies.get({ url: targetUrl });
            console.log(`[Main Process] Parent Session Verified Cookies for ${parsedUrl.hostname}:`);
            verifiedCookies.forEach(c => {
              console.log(`  Verified Cookie: name=${c.name}, value=${c.value.substring(0, Math.min(10, c.value.length))}..., domain=${c.domain}`);
            });
          }
...
function bindCookieMutatorToSession(sess) {
  ...
  sess.webRequest.onHeadersReceived((details, callback) => {
    ...
    if (setCookieKey && responseHeaders[setCookieKey]) {
      ...
      try {
        ...
        const isOAuthOrSystem = parsedUrl.hostname.includes('google.com') ||
                                parsedUrl.hostname.includes('googleusercontent.com') ||
                                parsedUrl.hostname.includes('gstatic.com') ||
                                parsedUrl.hostname.includes('facebook.com') ||
                                parsedUrl.hostname.includes('apple.com') ||
      ...
      console.log(`[Main Process] [Cookie Mutator] Intercepted Set-Cookie for App URL: ${details.url}`);
      responseHeaders[setCookieKey] = responseHeaders[setCookieKey].map(cookieStr => {
        ...
        if (!/domain=/i.test(modified)) {
          ...
          if (parts.length >= 2 && !parsedUrl.hostname.includes('localhost') && !parsedUrl.hostname.includes('127.0.0.1')) {
  ...
  sess.cookies.on('changed', (event, cookie, cause, removed) => {
    console.log(`[Cookie Event] name=${cookie.name}, domain=${cookie.domain}, path=${cookie.path}, value=${cookie.value.substring(0, Math.min(10, cookie.value.length))}..., cause=${cause}, removed=${removed}`);
  });
}
